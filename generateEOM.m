...
syms q1 q2 q3 q4 q5 dq1 dq2 dq3 dq4 dq5 ddq1 ddq2 ddq3 ddq4 ddq5 x dx ddx y dy ddy T1 T2 T3 T4 T5 Fpush  'real'; 
...
COM = (m1*G_1 + m2*G_2 + m3*G_3 + m4*G_4 + m5*G_5)/(m1 + m2 + m3 + m4 + m5);
dCOM = (m1*dG_1 + m2*dG_2 + m3*dG_3 + m4*dG_4 + m5*dG_5)/(m1 + m2 + m3 + m4 + m5);

l_pend = sqrt(COM(1)^2 + COM(2)^2);
xCOM = COM(1) + dCOM(1)/sqrt(g/l_pend);


COM_fcn = matlabFunction(COM(1:2),'File','COM.m');
dCOM_fcn = matlabFunction(dCOM(1:2),'File','dCOM.m');
xCOM_fcn = matlabFunction(xCOM,'File','xCOM.m');
